...
STREAM_ORDER: tuple[str, ...] = (
    ...
    "stage2e_test",
    "stage2e_bootstrap",
    # Stage 2E.1 appends its own stream so every earlier stage stays
    # bit-identical; it must never reuse a child of stage2e_bootstrap,
    # which the Brier resampling already spawns from.
    "stage2e1_bootstrap",
    # Stage 3A: a weaker signal (s = 0.6) alongside the s = 1 reference.
    # Appended, so every earlier stage stays bit-identical.
    "stage3a_calibration",
    "stage3a_test",
    "stage3a_bootstrap",
    # Stage 3A.1: the monitoring horizon itself becomes the variable.
    "stage3a1_calibration",
    "stage3a1_test",
    "stage3a1_bootstrap",
    # Stage 3B: a strategy that is valid first and fails later.
    "stage3b_test",
...
class Stage1Config:
    ...
    # Only the signal strength changes.  The two settings SHARE their noise
    # blocks (common random numbers) so the s = 1 and s = 0.6 results are paired;
    # calibration and test blocks are never shared with each other.
    stage3a_sharpes: tuple[float, ...] = (1.0, 0.6)
    stage3a_calibration_paths: int = 10000
    stage3a_test_paths: int = 10000
    stage3a_bootstrap_reps: int = 2000
    # the explanatory "probability has reached 0.9" marker; NOT a 10% FAR rule
    stage3a_prob_level: float = 0.9

    # --- Stage 3A.1: monitoring-horizon diagnostic --------------------------
    # Only the horizon the false-alarm budget is calibrated over moves.  No model
    # parameter is searched and no dynamic boundary is fitted.
    stage3a1_horizons: tuple[int, ...] = (63, 126, 252, 504)
    stage3a1_calibration_paths: int = 10000
    stage3a1_test_paths: int = 10000
    stage3a1_bootstrap_reps: int = 2000
    # This round has 4 x 2 x 2 x 2 x 4 = 128 rules, but keeps Stage 3A's
    # per-cell buffer delta/160 rather than the looser delta/128, so the
    # calibration is no weaker than the previous round's.  It is NOT a joint
    # guarantee across every stage of the project.
    stage3a1_delta_denominator: int = 160

    # --- Stage 3B: random failure time, a bounded diagnostic ----------------
    # theta_t = s for t <= T and 0 afterwards.  T is drawn independently of the
    # noise, the latent volatility and the jumps, and is visible only to the
    # generator and the scorer.  T <= 252 keeps a full 252-day post-failure
    # window inside the 504-day horizon; this is a "fails within the first
    # year" experiment, not a 0-5 or 0-10 year lifetime study.
    stage3b_fixed_failure_days: tuple[int, ...] = (0, 126, 252)
    stage3b_random_failure_max: int = 252
