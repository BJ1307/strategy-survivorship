...
def median_first_passage_day(tau: np.ndarray, horizon: int, sentinel: int = -1) -> tuple:
    """First day the cumulative alarm proportion over ALL paths reaches 1/2.

    This is the population median detection time, not the median among the paths
    that happened to alarm: a path that never alarms is not missing data, it is a
    detection time larger than the horizon.  Exactly one half counts as reached.

    ``sentinel`` marks "never alarmed" and must never be treated as a small time.
    Returns ``(day, note)``; ``day`` is ``""`` when the level is never reached.
    """
    t = np.asarray(tau)
    alarmed = t != sentinel
    n = t.size
    if n == 0:
        return "", "no paths"
    cum = np.array([float((alarmed & (t <= d)).sum()) / n for d in range(1, horizon + 1)])
    hit = np.nonzero(cum >= 0.5)[0]
    if hit.size:
        return int(hit[0]) + 1, ""
    return "", f"not reached within {horizon}d"
