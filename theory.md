...
`e_t/sqrt(V_t)` 近似标准正态时，被截断的更新项的期望为

```
    E[min(u^2, c^2)] = (2 Phi(c) - 1) - 2 c phi(c) + 2 c^2 (1 - Phi(c))
```

`c = 4` 时等于 0.99988，即长期方差目标低估 0.0121%。本项目**不做**偏差修正，
把它作为一个已知且被量化的偏差记录在案。

### 16.4 为什么交互项才是“是否取决于配对”的正确统计量
...
```
    ...
    交互项:                 [D(截断-t) - D(EWMA t)] - [D(截断-G) - D(EWMA G)]
```

“数哪一边显著的情境更多”不是对交互的检验：两边可以都显著且大小相同。
交互项显著为负对应“截断是重尾似然的替代品”，显著为正对应“两者必须配对”，
区间覆盖 0 则与“两条稳健化各自独立、近似可加”一致。区间由同一组 bootstrap 下标
（一次校准重抽 + 一次测试重抽，四个格子共用）得到，因此配对且包含门槛的抽样不确定性。
区间覆盖 0 时必须同时报告区间半宽，否则“没有交互”与“样本量看不出交互”无法分辨。
...
```
    类 1: k_tau >= 1                                  报警当日本身是跳跃日
    类 2: k_tau = 0 且 存在 s < tau, s >= tau - W, k_s >= 1    前 W 日内有跳跃
    类 3: 其余
```

`W = 20`。三类按构造互斥且并集等于 `{tau != -1}`，故三者之和恒等于总 FAR。

**这是时点关联，不是因果分解。** 在这个 DGP 里跳跃日与高波动日本身相关，
把类 1 的比例读成“跳跃造成的误杀份额”会把 SV 的贡献一并记到跳跃头上。

**自带的零对照。** `sv_jump_noise` 在 `kappa = 0` 时仍按同一强度 `lambda` 抽出跳跃计数，
只是幅度为 0（这也是让 `kappa = 0` 与纯 SV 生成器逐位相同的原因）。
因此两个 `kappa = 0` 情境里的类 1、类 2 量的纯粹是巧合重合，
给出“报警与跳跃无关时该看到多少”的精确基准：
每日跳跃概率 `lambda/D = 2/252 = 0.79%`，
20 日窗口的巧合覆盖率 `1 - (1 - 0.0079)^20 = 14.7%`，与实测一致。
读 `kappa > 0` 的结果时应当减去这个基准，而不是直接读绝对比例。
