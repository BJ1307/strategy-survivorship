...
def figure_gaussian(cfg, m: pd.DataFrame, out: Path) -> Path:
    """The iid-Gaussian control against the reference that actually applies there."""
    days = list(cfg.stage2c_report_days)
    sharpes = sorted(m.sharpe_valid.unique(), reverse=True)
    fig, axes = plt.subplots(1, len(cfg.far_targets), figsize=(11.4, 4.8), sharey=True)
    axes = np.atleast_1d(axes)
    for i, a in enumerate(sorted(cfg.far_targets, reverse=True)):
        ax = axes[i]
        for s in sharpes:
            col = "#1f77b4" if s == 1.0 else "#d62728"
            bound = [max_detection(s, d / cfg.D, a) for d in days]
            ax.plot(days, bound, ls="--", lw=1.2, color=col, alpha=0.55,
                    label=f"s={s:g}: nominal-budget reference")
            for mth in HEADLINE:
                ...
                ax.plot(days, [float(r[f"detect_d{k}"].iloc[0]) for k in days],
                        ls="-", ms=3.5, lw=1.0, color=col, alpha=0.75,
                        marker="o" if mth == "binary_gaussian" else ".")
    ...
    fig.suptitle("Stage 3A figure 4: the iid-Gaussian control against D_max(h, alpha) = "
                 "Phi(s*sqrt(h) - z_(1-alpha)).\nThe dashed curve uses the NOMINAL budget at "
                 "each horizon; our rules spend one budget across the whole two years, so "
                 "they cannot use the full alpha at every horizon.", y=1.0, fontsize=10)
    fig.tight_layout()
    fig.savefig(out, dpi=160, bbox_inches="tight")
    plt.close(fig)
    return out
