...
def _summary(tau: np.ndarray, cfg: Stage1Config, H: int) -> dict:
    ...
    n = int(tau.size)
    k = int(alarmed.sum())
    trunc = np.where(alarmed, tau, H).astype(float)
    med = float(np.median(tau[alarmed])) if k > n / 2 else float("nan")
    return {"n_paths": n, "n_alarms": k, "rate": k / n,
            ...
            "median_first_alarm": med,
            "median_note": "" if med == med else f"not reached within H={H}",
